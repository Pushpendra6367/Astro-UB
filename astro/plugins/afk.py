...
afk_start = {}
BOTLOG = True
CUSTOM_AFK = Config.CUSTOM_AFK if Config.CUSTOM_AFK else "I am currently unavailable!"
...
@astro.on(admin_cmd(pattern=r"afk ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
    global USER_AFK
    ...
    if not USER_AFK:
        if event.reply_to_msg_id:
            ...
            media = await astro.download_media(reply_message, "AFK_media")
            try:
...
@astro.on(events.NewMessage(outgoing=True))
