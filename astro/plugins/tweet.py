...
@astro.on(admin_cmd(pattern="btweet(?: |$)(.*)"))
@astro.on(sudo_cmd(pattern="btweet(?: |$)(.*)", allow_sudo=True))
async def teletweet(astro):
    # """Creates random anime sticker!"""
    what = astro.pattern_match.group(1)
    if not what:
        if astro.is_reply:
            what = (await astro.get_reply_message()).message
        else:
            await eor(astro, "`Tweets must contain some text, pero!`")
            return
    sticcers = await bot.inline_query("TwitterStatusBot", f"{(deEmojify(what))}")
    await sticcers[0].click(
        astro.chat_id,
        reply_to=astro.reply_to_msg_id,
        silent=True if astro.is_reply else False,
        hide_via=True,
    )
    await astro.delete()
...
@astro.on(admin_cmd(pattern="tweet ?(.*)"))
@astro.on(sudo_cmd(pattern="tweet ?(.*)"))
async def handler(event):
    ...
    await astro.send_file(event.chat_id, pic_tweet, reply_to=reply_to)
    await event.delete()
