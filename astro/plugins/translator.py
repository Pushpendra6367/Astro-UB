...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        await edit_or_reply(event, "`.tr LanguageCode` as reply to a message\n\nCheck Languages Codes at [Astro_HelpChat](https://t.me/Astro_HelpChat/2197)")
        return
    lan = lan.strip()
    try:
        lmao_bruh = text
        ...
        output_str = f"""**TRANSLATED SUCCESSFULLY BY Astro🌌**
**Source ({source_lan})**:
`{text}`
...
\n**Usage :** Translates the given text into your language. language codes are here [Astro](https://t.me/Astro_HelpChat/2197)"
    }
)